...
window.onload=function(){try{var isDown=!1;var offset=0;var handle=null;var valueFrom=document.querySelector('.double-range .min-start').innerHTML;var valueTo=document.querySelector('.double-range .max-end').innerHTML;var handles=document.querySelectorAll('.slider-handle');var handleFrom=document.querySelector('.slider-handle.price-from');var handleTo=document.querySelector('.slider-handle.price-to');var sliderRange=document.querySelector('.slider-range')}catch(e){console.log('селектор цены не нашли на страничке')}
...
function mouseDown(event){if(event.button===0){isDown=!0;handle=event.currentTarget;offset=event.currentTarget.offsetLeft-event.clientX+10;document.addEventListener('mouseup',function(){if(event.button===0){isDown=!1}},!0)}}
...
handleTo.style.left=moveDistance+'%';updateValues(handleTo);sliderRange.style.width=parseInt(handleTo.style.left)-parseInt(handleFrom.style.left)+'%'}}},!0);var formFields=document.querySelectorAll('.form-contact input[type="text"], .form-contact textarea, .form-contact input[type="email"]');var formSubmit=document.querySelector('.form-contact input[type="submit"]');if(formSubmit){formSubmit.addEventListener('click',validateAll,!1)}
...
function validate(event){event.currentTarget.className+=" validate"}
function validateAll(event){for(var i=0;i<formFields.length;i++){formFields[i].className+=" validate"}}
...
var contactLink=document.querySelector('.form-contact-link');var formOverlay=document.getElementById('popup-writeus');if(contactLink){contactLink.addEventListener('click',function(event){formOverlay.classList.add('show');formOverlay.addEventListener('click',function(event){if(event.target==formOverlay){formOverlay.classList.remove("show")}});event.preventDefault();window.addEventListener("keydown",function(event){if(event.keyCode===27){if(formOverlay.classList.contains("show")){formOverlay.classList.remove("show")}}});var formClose=formOverlay.querySelector('.modal-close-btn');if(formClose){formClose.addEventListener('click',function(event){formOverlay.classList.remove('show');event.preventDefault()})}},!1)}}
